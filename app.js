...
const fields = {
  ...
  hp: document.getElementById("hp"), // Hidden honeypot field/bot detection only)
};
...
const clearBtn = document.getElementById("clearBtn"); // clear btn

// 2. display company fields when checkbox is clicked

//  click the "Register as company" checkbox...
...
// 3. Debounce to minimize validations while typing

// set debounce while typing
...
// 4. Validation functions one for each
...
function validateName() {
  ...
  // check name is at least 2 chars.
...
function validateCompany() {
  ...
  if (el.validity.valueMissing) {
    el.setCustomValidity(
      "Company name is required when registering as a company."
    );
  }
...
// 5. Live validation of input fileds using debounce

// Run validation when user types in each field
fields.name.addEventListener("input", () => {
  debounce(validateName, 150)();
  buildSummary();
});
fields.email.addEventListener("input", () => {
  debounce(validateEmail, 150)();
  buildSummary();
});
fields.password.addEventListener("input", () => {
  debounce(validatePassword, 150)();
  buildSummary();
});
fields.phone.addEventListener("input", () => {
  debounce(validatePhone, 150)();
  buildSummary();
});
fields.company.addEventListener("input", () => {
  debounce(validateCompany, 150)();
  buildSummary();
});

// 6.  Error summary at the top of the form element
...
// 7. Auto save data using local storage
...
// 9. Form submission/ final validation and submit
